...
* **Biometric Sign-In Duress Gap (F5):** A WebAuthn/biometric login tap produces no keystroke dynamics. ARES therefore receives all-null behavioral signals and cannot distinguish coerced from voluntary biometric use. Mitigation (implemented): a 2-second hold gesture before the platform authenticator prompt fires sets `manualDuressSignal = true`, providing the same manual escape hatch available on the password path. This is a UX mechanism, not a behavioral one — it relies on the user's ability to hold the button for 2 seconds undetected. Future work: integrate a hold-force or micro-tremor signal from device accelerometer/touch pressure APIs as an implicit biometric duress indicator.
* **Master PIN — Physical Coercion ($5-Wrench) Weakness (F3):** The dual-PIN scheme (normal PIN → decoy, master PIN → authentic) provides plausible deniability only while the adversary is unaware that a master PIN exists. A sophisticated coercer may demand both PINs sequentially, or demand the user prove there is no second PIN. This is the well-known "$5-wrench attack" (XKCD #538): no cryptographic protocol fully solves it. The PIN gate is a usability convenience for casual threats; ARES's behavioral trigger remains the primary countermeasure for adversarial scenarios. Users should be informed that the PIN gate does not provide strong security against an informed, persistent attacker.
* **Admin Panel Information Hazard (F4):** The admin dashboard shows login timestamps, device info, and session counts. While Session.state, RiskEvent, PolicyDecision, and BehavioralEvent are deliberately excluded via Prisma field allowlists, correlation across login timestamps and IP addresses could still reveal patterns. A compromised admin account represents a partial information hazard. Mitigated by: OTP re-verification requirement (5-minute freshness), AdminAccessLog auditing all views before data is fetched, and field-allowlist enforcement at the query layer.
